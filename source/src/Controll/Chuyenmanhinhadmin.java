...
public class Chuyenmanhinhadmin {
    ...
    public void setView(JPanel jpnItem, JLabel jlbItem) {
        ...
        root.add(new PN_Admin_Trangchu());
        root.validate();
        root.repaint();
    }
    ...
    class LabelEvent implements MouseListener {
        ...
        public void mouseClicked(MouseEvent e) {
            switch (kind) {
            case "trangchu":
                node = new PN_Admin_Trangchu();
                break;
            case "taikhoan":
                node = new PN_Admin_Taikhoan();
                break;
            case "diemsinhvien":
                node = new PN_Admin_Bangdiem();
                break;
            case "lichhoc":
                node = new PN_Admin_Lichhoc();
                break;
            case "diemdanh":
            	node = new PN_Admin_Diemdanh();
                break;
            case "danhsachsinhvien":
            	node = new PN_Admin_Danhsachsinhvien();
                break;
            default:
                node = new PN_Admin_Trangchu();
            }
        ...
        public void mouseExited(MouseEvent e) {
            if (!kindSelected.equalsIgnoreCase(kind)) {
                jpnItem.setBackground(new Color(44, 130, 201));
                jlbItem.setBackground(new Color(44, 130, 201));
            }
        }
    ...
    private void changeBackGroundColor(String kind) {
        for (DanhMuc item : listItem) {
            if (item.getKind().equalsIgnoreCase(kind)) {
                ...
                item.getJlb().setBackground(new Color(44, 130, 201));
                item.getJpn().setBackground(new Color(44, 130, 201));
            }
        }
    }
}
